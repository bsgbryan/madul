...
const doWrap = (spec, instance, method, output) =>
  async params =>
    new Promise(async (resolve, reject) => {
      const progress = params => emit.progress(spec, method, params)
      const done     = async output => {
        emit.done(spec, method, output)

        if (await execute({ mode: 'after', output, spec, method }))
          resolve(output)
      }

      try {
        emit.call(spec, method, params)

        if (await execute({ mode: 'before', params, spec, method }))
          instance[method].call(output, { ...params, done, progress })
      }
      catch (e) {
        emit.error(spec, method, e)
        reject(e)
      }
    })
