          ...
          @[method] = =>
            ...
            respond = (state, data) ->
              process.nextTick ->
                ...
                if state == 'reject'
                  logger.rejected self.clazz, method, microtime.now() - start, data

                if process.env.MOTHERBRAIN_LOG_LEVEL > 4
                  if state == 'resolve'
                    logger.resolved self.clazz, method, microtime.now() - start, data
                  else if state == 'notify'
                    logger.notify self.clazz, method, microtime.now() - update, data
                    update = microtime.now()

            self[key] = val for key, val of @

            self.done   = (out) -> respond 'resolve', out
            self.fail   = (err) -> respond 'reject',  err
            self.update = (out) -> respond 'notify',  out

            args = Array.prototype.slice.call arguments

            logger.trace "Invoking #{@clazz}.#{method}()", args

            @["_#{method}"].apply self, args
