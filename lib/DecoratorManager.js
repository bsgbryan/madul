...
const loadFromAllBundles = async spec => {
  ...
  if (status.isDirectory()) {
    await each(await readdir(root), async node => loadFromBundle(spec, node))
  }
}
...
const set = async (spec, list) => {
  const initialize = require('./Initializer')

  decorators[spec] = await map(list, async l => await initialize(l))
}
...
const add = async (spec, decorator) => {
  ...
  if (decorators[spec].some(d => d.spec === decorator) === false)
    decorators[spec].push({
      spec:  decorator,
      madul: await initialize(decorator)
    })
  else
